...
(function () {
  ...
  function socialReach(content) {
    var r = (content.captain && content.captain.reach) || {};
    var social = content.social || {};
    var rows = [
      { value: r.facebookFollowers || "1.3M", platform: "Facebook", label: "Solomon Pagunsan", href: social.facebook },
      { value: r.facebookSecondaryFollowers || "238K", platform: "Facebook", label: "Solomon Retada Pagunsan", href: social.facebookSecond },
      { value: r.youtubeSubscribers || "1.02K", platform: "YouTube", label: "Subscribers", href: social.youtube },
      { value: r.tiktokFollowers || "14.8K", platform: "TikTok", label: "Followers", href: social.tiktok }
    ].filter(function (item) { return item.value && String(item.value).trim(); });
    if (!rows.length) return "";
    var items = rows.map(function (item) {
      var body = '<span class="social-stat__value">' + item.value + '</span>' +
        '<span class="social-stat__platform">' + item.platform + '</span>' +
        '<span class="social-stat__label">' + item.label + '</span>';
      return item.href && item.href.trim()
        ? '<a class="social-stat reveal" href="' + item.href + '" target="_blank" rel="noopener">' + body + '</a>'
        : '<div class="social-stat reveal">' + body + '</div>';
    }).join("");
    return '<section class="social-reach" aria-label="Social media audience"><div class="social-reach__inner">' + items + '</div></section>';
  }



  /* ===================== HOME AUTHORITY LAYER ==================== */
  function homeAuthority(content) {
    var h = content.homeAuthority || {};
    var cards = (h.cards || []).map(function (item) {
      return '<a class="authority-card reveal" href="' + (item.href || '#') + '">' +
        '<h3>' + (item.title || '') + '</h3>' +
        '<p>' + (item.text || '') + '</p>' +
        '<span>Read more &rarr;</span></a>';
    }).join("");
    if (!h.title && !cards) return "";
    return '<section class="authority" id="official-profile">' +
      '<div class="authority__intro reveal">' +
        '<p class="kicker">' + (h.kicker || 'Official Profile') + '</p>' +
        '<h2 class="section-title">' + (h.title || '') + '</h2>' +
        '<p class="authority__lead">' + (h.lead || '') + '</p>' +
      '</div>' +
      '<div class="authority__grid">' + cards + '</div></section>';
  }

  function homeSpotlight(content) {
    var s = content.homeSpotlight || {};
    if (!s.title) return "";
    return '<section class="story-spotlight" id="student-story">' +
      '<figure class="story-spotlight__media reveal" ' + bgStyle(s.image, 'linear-gradient(160deg,var(--green-700),var(--cane))') + '></figure>' +
      '<div class="story-spotlight__body reveal">' +
        '<p class="kicker">' + (s.kicker || '') + '</p>' +
        '<h2 class="section-title">' + (s.title || '') + '</h2>' +
        '<p>' + (s.text || '') + '</p>' +
        '<a href="' + (s.href || 'farm-learning.html') + '" class="btn btn--solid">' + (s.ctaLabel || 'Learn more') + '</a>' +
      '</div></section>';
  }

  function materialsSpotlight(content) {
    var s = content.materialsSpotlight || {};
    if (!s.title) return "";
    return '<section class="materials-strip" id="materials-strip">' +
      '<div class="materials-strip__body reveal">' +
        '<p class="kicker kicker--light">' + (s.kicker || '') + '</p>' +
        '<h2 class="section-title section-title--light">' + (s.title || '') + '</h2>' +
        '<p>' + (s.text || '') + '</p>' +
        '<a href="' + (s.href || 'products.html') + '" class="btn btn--gold">' + (s.ctaLabel || 'View materials') + '</a>' +
      '</div>' +
      '<figure class="materials-strip__media reveal" ' + bgStyle(s.image, 'linear-gradient(160deg,var(--soil),var(--green-700))') + '></figure>' +
      '</section>';
  }
  ...
  /* ============================ WORKSHOP ========================= */
  function workshopSection(content) {
    ...
    return '<section class="workshop workshop--embedded" id="workshop">' +
  ...
  function farmLearningPage(content) {
    ...
    return '<section class="visits" id="student-visits"><div class="visits__top">' +
      ...
      '</div>' +
      (gal ? '<div class="learn-gallery" aria-label="Student visit photos">' + gal + '</div>' : "") +
  ...
  function partnersPage(content) {
    ...
    var galleryItems = Array.isArray(gallery.items) ? gallery.items : [
      { img: "images/Sponsor_1.jpg", partner: "JI Telecom", caption: "A partner collaboration with Solomon.", active: true },
      { img: "images/Sponsor_3.jpg", partner: "MMTT Laptop Services", caption: "Meeting and collaboration with a local technology partner.", active: true },
      { img: "images/Sponpor_3.jpg", partner: "GoTraktora", caption: "Agricultural partnership and product activity in Negros.", active: true }
    ];
    var galleryCards = galleryItems.filter(function (item) {
      return item && item.active !== false && item.img && item.img.trim();
    }).map(function (item) {
      return '<figure class="partner-gallery__item reveal" data-lightbox-img="' + item.img + '">' +
        '<img src="' + item.img + '" alt="' + esc(item.caption || item.partner || "Partner activity") + '" loading="lazy" decoding="async">' +
        ((item.partner || item.caption) ? '<figcaption>' +
          (item.partner ? '<strong>' + item.partner + '</strong>' : "") +
          (item.caption ? '<span>' + item.caption + '</span>' : "") + '</figcaption>' : "") +
      '</figure>';
    }).join("");
    ...
    return pageBanner(p.kicker, p.title, p.sub) +
      '<section class="sponsors" id="partners">' + sponsorsMarquee(content, false) +
        (galleryCards ? '<div class="partner-gallery">' +
          '<div class="partner-gallery__head">' + head(gallery.kicker || "Partnerships in Action", gallery.title || "Working together, on and beyond the farm", gallery.intro || "") + '</div>' +
          '<div class="partner-gallery__grid">' + galleryCards + '</div></div>' : "") +
  ...
  /* ===================== PAGE COMPOSERS ========================= */
  var pages = {
    home: function (c) { return hero(c) + stats(c) + socialReach(c) + homeSpotlight(c) + homeAuthority(c) + materialsSpotlight(c) + teasers(c) + announcements(c) + partnersStrip(c) + contactCta(c); },
